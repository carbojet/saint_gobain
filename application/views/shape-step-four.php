...
<html>
	...
	<body>
						...
						<div class="col-12">
							<div class="site-box-div bg-white">
								...
								<div class="site-btn-div mt-4 pb-4 pt-2 text-center">
									...
									<button type="button" class="site-btn tab-box-btn active px-5 f-16" id="tab_btn_2">Process Parameter</button>
									<?php }else{ ?>
									<button type="button" class="site-btn tab-box-btn px-5 active f-16 mr-5" id="tab_btn_1">Preview</button>
									...
									<div class="pre-dwn-img-inner site-shadow">
										...
										<div class="mt-3">
											<ul class="pre-down-btn">
												<li>
													<a href="<?php echo site_url('sentinal/shape/step-three.html?row_id='.$row_id.'&plant_id='.$plant_id.'&is_edit=1');?>" class="site-btn">Edit</a>
												</li>
								...
								<div class="s-tab-box <?php if(!empty($job)){ if($job[0]->jobstatus == "FALSE"){ ?>active<?php }}else{ ?>active<?php } ?>" id="tab_btn_2_box">
									<form action="<?php echo site_url('sentinal/save_shape_process_paremeter_data');?>" method="post" autocomplete="off">	
		...
		<div class="modal fade" id="prevSubmitModal" tabindex="-1" aria-labelledby="prevSubmitModalLabel" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
						<p class="modal-title f-16 mx-auto" id="prevSubmitModalLabel">Do you want to Create Another Drawing</p>
					</div>
					<div class="modal-body">
						<ul class="site-input-list mx-auto grid-2 clearfix pt-3">
							<li class="input-list-li">							
								<div class="form-group filter-sel">
									<label class="input-label">Module</label>
									<select class="form-control custom-select s-input-border" id="sales_order_module">
										<option value="">Select Module</option>
										<?php foreach($sales_order_module as $order){ ?>
										<option value="<?php echo $order->module;?>"><?php echo $order->module;?></option>
										<?php } ?>
									</select>
								</div>
							</li>
							<li class="input-list-li">							
								<div class="form-group filter-sel">
									<label class="input-label">Line Item</label>
									<select class="form-control custom-select s-input-border" id="line_row_id">
										<option value="">Select Line Item</option>
										<?php foreach($line_item as $item){ ?>
										<option value="<?php echo $item->row_id;?>"><?php echo $item->sale_line_no;?></option>
										<?php } ?>
									</select>
								</div>
							</li>
						...
						<div class="site-btn-div mt-4 pb-4 pt-2 text-center">
							<button type="button" class="site-btn px-5 f-16 mr-4" id="redirct_process">Yes</button>
							<button type="button" class="site-btn px-5 f-16 ml-4" data-dismiss="modal">No</button>
						</div>
					</div>
				</div>
			</div>
		...
		<script>
			$(document).ready(function(){
				...
				$('#download-all').on('click' , function(){
					downloadAll(links);
					$("#prevSubmitModal").show();
				});

				$(document).on( "click", ".custom-radios > li", function() {
					$('.custom-radios > li').removeClass('active');
					$('#line_row_id').val($(this).data('rowid'));
					$(this).addClass('active');
				});
				$(document).on( "click", "#redirct_process", function() {
					if($("#line_row_id").val() != ''){
						var url = "<?php echo base_url('sentinal/shape/step-one.html?row_id=');?>"+$("#line_row_id").val()+"&plant_id=<?php echo $plant_id;?>";
				...
				$('#sales_order_module').on('change', function() {
                    $("#line_row_id option").remove();
                    
                    var order_module = $(this).find(':selected').val();
                    var sale_ord_no = '<?php echo $salesOrder[0]->sale_ord_no; ?>';
                    var oa_num = '<?php echo $salesOrder[0]->oa_num; ?>';
                    var row_id = '<?php echo $row_id;?>';
                    var plant_id = '<?php echo $plant_id;?>';

                    $.ajax({
                        url: "<?php echo site_url("sentinal/get_line_item_by_condition");?>",
                        type: "POST",
                        dataType:'json',
                        data:{order_module:order_module,sale_ord_no:sale_ord_no,oa_num:oa_num,row_id:row_id,plant_id:plant_id},
                        async: false,
                        success: function(data){
                            $('#line_row_id').append($('<option>', {
                                text: "Selec Line Item",
                                value: ""
                            }));
                            $.each(data, function(index, element) {
                                $('#line_row_id').append($('<option>', {
                                    text: element.sale_line_no,
